...
public class Main_15685_드래곤커브 {
	...
	static int N, r, c, n, d, endr, endc, resr, resc;
	static int[][] direc = {{1,0},{0,-1},{-1,0},{0,1}};
	static int[][] arr = new int[101][101];
	public static void main(String[] args) throws Exception{
		BufferedReader br = new BufferedReader(new FileReader("res/Main_15685_드래곤커브.txt"));
//		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		...
		for(int i=0; i<N; i++) {
			...
			arr[r][c] = 1;
			set.add(new XY(r,c));
			arr[r+direc[d][0]][c+direc[d][1]] = 1;
			
			endr = r+direc[d][0];
			endc = c+direc[d][1];
			set.add(new XY(endr,endc));
			for(int j=0; j<n; j++) {
				HashSet<XY> newset = new HashSet<>(set);
				for(XY xy:newset) {
					Rotation(xy.x,xy.y);
					arr[resr][resc] = 1;
					set.add(new XY(resr, resc));
				}
				Rotation(r,c);
				endr = resr;
				endc = resc;
	...
	private static void Rotation(int x, int y) {
		double newDegree = 90;
		double degree = Math.toRadians(newDegree);
		double cos = Math.cos(degree);
		double sin = Math.sin(degree);
		double sr = x-endr;
		double sc = y-endc;
		double rr = (sr*cos - sc*sin)+endr;
		double rc = (sr*sin + sc*cos)+endc;		 	
		resr = (int)Math.round(rr);
		resc = (int)Math.round(rc);
	}
}
