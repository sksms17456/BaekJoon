...
public class Main_2503_숫자야구 {
	static int N;
	static String[] arr=new String[504];//가능한 숫자배열
	static int TC=0;
	static int[] numbers;
	static int stk, ball;
	static String basenum="";
	public static void main(String[] args) throws Exception{
		BufferedReader br = new BufferedReader(new FileReader("Main_2503_숫자야구.txt"));
//		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
			...
			for(int i=0; i<arr.length; i++) {
				if(arr[i]!=null) {
					...
					for(int base=0; base<3; base++) {
						char comp = basenum.charAt(base);
						for(int arrn=0; arrn<3; arrn++) {
							if(arr[i].charAt(arrn)==comp) {
								if(base==arrn) {
									stkcnt++;
								}
								else {
	...
	public static void numlist(int s, int next) {
		if(s==3) {
			String str="";
