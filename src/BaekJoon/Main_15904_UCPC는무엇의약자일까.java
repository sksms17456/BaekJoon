...
public class Main_15904_UCPC는무엇의약자일까 {
	static boolean isUCPC;
	static String input;
	static char[] ucpcKey = { 'U', 'C', 'P', 'C' };
	static String[] ans = { "I love UCPC", "I hate UCPC" };

	public static void main(String[] args) throws Exception {
		BufferedReader br = new BufferedReader(new FileReader("res/Main_15904_UCPC는무엇의약자일까.txt"));
//		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		input = br.readLine();
		abbreviate(0, 0);
		System.out.println(isUCPC ? ans[0] : ans[1]);
	}

	static void abbreviate(int idx, int key) {
		if (key == 4) {
			isUCPC = true;
			return;
		}
		for (int i = idx, len = input.length(); i < len; i++) {
			if (input.charAt(i) == ucpcKey[key]) {
				abbreviate(i + 1, key + 1);
				return;
			}
		}
	}
