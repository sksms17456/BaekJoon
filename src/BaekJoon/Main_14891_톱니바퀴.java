...
public class Main_14891_톱니바퀴 {
	static int N, n, dir, sum;
	static int[] check = new int[5];
	static String line;
	static ArrayList<Integer>[] gear = new ArrayList[5];
	public static void main(String[] args) throws Exception{
		BufferedReader br = new BufferedReader(new FileReader("res/Main_14891_톱니바퀴.txt"));
//      BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
	...
	private static void rotate(int n, int dir, int v) {
		check[n] = v;
		int left = n-1;
		if(left>0 && check[left]!=v && gear[n].get(6)!=gear[left].get(2)) {
			rotate(left,dir*-1,v);
		}
		int right = n+1;
		if(right<5 && check[right]!=v && gear[n].get(2)!=gear[right].get(6)) {
			rotate(right,dir*-1,v);
		}
		if(dir==1) {
			gear[n].add(0, gear[n].get(7));
			gear[n].remove(8);
		}else {
			gear[n].add(gear[n].get(0));
			gear[n].remove(0);
		}
	}
}
