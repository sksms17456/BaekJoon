...
public class Main_1922_네트워크연결 {
	static int N,M,costsum=0;
	static int[][] com;
	static int[] cost;
	static int min=Integer.MAX_VALUE;
	public static void main(String[] args) throws Exception{
		BufferedReader br = new BufferedReader(new FileReader("Main_1922_네트워크연결.txt"));
//		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		...
		for(int m=0; m<M; m++) {
			...
			com[m][0] = Integer.parseInt(rd[0]);
			com[m][1] = Integer.parseInt(rd[1]);
			cost[m] = Integer.parseInt(rd[2]);
		}
		
		network(com[0][0],com[0][1],0,0);
		System.out.println(min);
	}
	public static void network(int start, int end, int idx,int costsum) {
		costsum+=cost[idx];
		System.out.println(start+" "+end+" "+idx+" "+costsum);
		if(end==N) {
