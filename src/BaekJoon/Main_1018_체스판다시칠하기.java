...
//
//어떤 정사각형은 검정색으로 칠해져있고, 나머지는 흰색으로 칠해져 있다. 지민이는이 보드를 잘라서 8*8크기의 체스판으로 만들려고 한다.
//
...
public class Main_1018_체스판다시칠하기 {
	static int M, N, mincnt = Integer.MAX_VALUE, wcnt, bcnt;
	static char[][] Chess;
	public static void main(String[] args) throws Exception{
		BufferedReader br = new BufferedReader(new FileReader("res/Main_1018_체스판다시칠하기.txt"));
//		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
	...
	private static void WColoring(int r, int c) {
		wcnt = 0;
		Top:
		for(int i=r; i<r+8; i++) {
			for(int j=c; j<c+8; j++) {
	...
	private static void BColoring(int r, int c) {
		bcnt = 0;
		Top:
		for(int i=r; i<r+8; i++) {
			for(int j=c; j<c+8; j++) {
